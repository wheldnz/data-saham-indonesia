...
echo   4. Menjalankan pipeline awal (download data 941 emiten IDX)
...
if %ERRORLEVEL% neq 0 (
    ...
    echo [ERROR] Unduh dari: https://www.python.org/downloads/
...
if %ERRORLEVEL% neq 0 (
    ...
    echo [ERROR] Unduh dari: https://nodejs.org/
    pause
    exit /b 1
)
...
if not exist "node_modules" (
    echo Menginstall packages Node.js...
    npm install
    if %ERRORLEVEL% neq 0 (
...
echo ------------------------------------------------------------
echo Proses ini bisa memakan waktu 10-30 menit. Jangan tutup!
echo ------------------------------------------------------------
echo.
cd /d "%~dp0backend"
call "%~dp0backend\venv\Scripts\activate.bat"
