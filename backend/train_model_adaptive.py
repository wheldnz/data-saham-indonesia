...
CV_REPORT_T1_PATH = os.path.join(DATA_DIR, 'cv_report_t1_adaptive.json')
CV_REPORT_T3_PATH = os.path.join(DATA_DIR, 'cv_report_t3_adaptive.json')

def train_model_adaptive():
    if not os.path.exists(DATASET_PATH):
        print(f"Dataset not found at {DATASET_PATH}. Please run prepare_ml_data_adaptive.py first.")
        return

    print(f"Loading adaptive dataset from {DATASET_PATH}...")
    ...
    print(f"Adaptive Dataset loaded. Total rows: {len(df)}")
    ...
    for horizon, cfg in targets_config.items():
        ...
        print(f"\n{'='*60}")
        print(f"  STARTING TRAINING FOR ADAPTIVE HORIZON: {horizon} (Target: {target_col})")
        print(f"{'='*60}")
